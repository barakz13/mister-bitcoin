...
export class ContactService {
  ...
  private _filterBy$ = new BehaviorSubject<ContactFilter>({
    name: '',
    phone: '',
  });
  ...
  public loadContacts(filterBy: ContactFilter | null): void {
  ...
  private _filter(contacts: Contact[], filterBy: ContactFilter) {
    return contacts.filter((contact) => {
      return (
        contact.name.toLocaleLowerCase().includes(filterBy.name) &&
        contact.phone.includes(filterBy.phone)
      );
    });
  }
  ...
  //MakiShift ID
  private _makeId(length: number = 5): string {
    var txt = '';
    var possible =
